locals {
    config_dir = "home/${var.user}/.config"
    ...
    boot_command = [
        ...
        "mkdir -p /mnt/${local.config_dir}/nixpkgs<enter><wait>",
        "curl http://{{ .HTTPIP }}:{{ .HTTPPort }}/home.nix > /mnt/${local.config_dir}/nixpkgs/home.nix<enter><wait10s>",
        ...
        "chown -R 1000:users /mnt/${local.config_dir}<enter><wait>",
        "reboot<enter>"
    ]
